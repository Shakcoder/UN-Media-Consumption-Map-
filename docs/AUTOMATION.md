...
**Daily**, GitHub runs the trend engine: it reads Wikipedia attention data and GDELT news coverage for 183 UN-relevant topics in 22 languages, computes what's rising where, and publishes the result. **Weekly** (Monday 03:00 UTC), a second workflow pulls 15 World Bank indicators and 4 ITU DataHub indicators plus per-country language data and rebuilds `data/countries.json`, then refreshes the Statcounter platform shares in `data/platform_web_shares.json`. **Monthly** (the 3rd), a watchdog checks whether RSF, Freedom House, Reuters, GSMA, Afrobarometer, UN DESA, or the WPP Media/Dentsu ad forecasts have published a new annual edition — and opens an Issue with step-by-step refresh instructions when they have. GitHub Pages republishes the site automatically after every data commit. If any workflow fails, it opens an Issue so failures are never silent.
...
|---|---|---|---|
| [`trend-engine.yml`](../.github/workflows/trend-engine.yml) | Daily 16:05 UTC (moved from 05:30 on 2026-08-11 — Wikimedia only publishes "yesterday's" per-article pageviews late morning UTC, so the early run always missed the newest day; the Colombia-earthquake spike made this visible) | Runs as **parallel lanes since 2026-08-10** (each lane gets its own runner, i.e. its own IP and rate budget — one runner could not carry a whole day's fetching). Supply lane: each country's trending Google searches (Trending Now RSS — first, because the feed keeps no history), the global Bluesky pulse, GDELT coverage, each country's most-read Wikipedia pages. Press lane: each country's UN-share of national-press stories (Media Cloud — needs the `MEDIACLOUD_API_KEY` repo secret, set 2026-08-10 from the team account) and each country's measured news-site censorship (OONI, keyless; CC BY-NC-SA — non-profit use only). Demand lanes: Wikipedia pageviews in 3 stalest-first shards. An assemble job merges everything, applies the access-method anomaly gate (2026-08-17: quarantines non-organic pageview series like France's fr-"5G" mobile-web flood — see the GATE_* block in fetch_trends_wikipedia.py; `--gate-only` re-runs just the gate by hand), extends the long-term attention archive (2026-08-18: per-topic global daily totals back to 2024, kept cell-for-cell equal to the pageviews file on shared days), computes the trend intelligence, rebuilds the movers RSS feed (`scripts/build_movers_feed.py` — the Topic Explorer's daily risers-and-fallers list as a subscribable feed, published figures verbatim, one item per day, 30 days kept; if the feed step ever fails the day's data still commits and the feed is just one day older), makes the single daily commit, and fails loudly if anything is going stale | `data/trends/country_searches.json`, `data/trends/bluesky_trends.json`, `data/trends/press_un_coverage.json`, `data/trends/wiki_pageviews.json`, `data/trends/attention_archive.json`, `data/trends/gdelt_coverage.json`, `data/trends/topic_intelligence.json`, `data/trends/country_reading.json`, `data/trends/movers_feed.xml` |
| [`trends-pulse.yml`](../.github/workflows/trends-pulse.yml) | 3× daily (05:30, 11:30, 22:30 UTC) | Extra snapshots of the two history-less "trending right now" feeds (Google Trends per-country searches, Bluesky global pulse). Added 2026-08-11: one snapshot a day at 00:30 Bogotá time missed the Colombia earthquake's entire search wave — these feeds rotate all day and forget everything, so each snapshot is UNION-merged into the day's archive. Shares the trend-engine concurrency group so the two never interleave commits | `data/trends/country_searches.json`, `data/trends/bluesky_trends.json` |
| [`refresh-data.yml`](../.github/workflows/refresh-data.yml) | Weekly, Mon 03:00 UTC + whenever `static_countries.json` or `refresh_data.py` changes | World Bank indicators (population, GDP, internet, literacy, Findex financial accounts, …) + ITU DataHub indicators (TV/radio households, 4G coverage, data affordability — via the World Bank Data360 mirror, CC BY 4.0) + Unicode CLDR language shares; then Statcounter social-platform web shares | `data/countries.json`, `data/platform_web_shares.json` |
...
| [`attention-archive-backfill.yml`](../.github/workflows/attention-archive-backfill.yml) | **Manual only** (Run workflow button) — ran once at launch (2026-08-18); run again only after adding topics to the registry, or if the archive file is ever lost | The Topic Explorer's long-term attention archive: global daily totals per topic back to 2024-01-01, fetched from the Wikimedia per-article API in 12 parallel shards (same own-IP-per-runner trick as the daily engine — one IP gets ~500 requests/hour before throttling, the backfill needs ~3,400; starved pairs retry in rounds while the budget refills). The assemble job refuses to write anything incomplete, golden-checks the result against `wiki_pageviews.json`, validates, and commits. After that the daily engine keeps the file current | `data/trends/attention_archive.json` |

All six also have a manual **Run workflow** button: GitHub → Actions tab → pick the workflow → Run workflow.
...
| `data/trends/country_reading.json` | daily trend engine, via `scripts/fetch_trends_wiki_countries.py` | daily, after the 16:05 UTC run (plus intraday pulse snapshots for searches/Bluesky) | Map — the "Most-read Wikipedia pages" block on a country's Media tab; AI Analyst — reading-list evidence in country answers |
| `data/trends/country_searches.json` | daily trend engine, via `scripts/fetch_trends_google.py` | daily, after the 16:05 UTC run (plus intraday pulse snapshots for searches/Bluesky) | Map — the "Trending searches" block on a country's Media tab; AI Analyst — the "What X is searching for" line and evidence in country answers |
| `data/trends/bluesky_trends.json` | daily trend engine, via `scripts/fetch_trends_bluesky.py` | daily, after the 16:05 UTC run (plus intraday pulse snapshots for searches/Bluesky) | Topic Explorer — the "Open social web right now" section on the overview pane (global only, never on country pages) |
| `data/trends/press_un_coverage.json` | daily trend engine, via `scripts/fetch_press_un_coverage.py` (needs the `MEDIACLOUD_API_KEY` repo secret; country→collection mapping lives in `data/sources/mediacloud_collections.json`) | daily, after the 16:05 UTC run (rolling 7-day window) | Map — the "UN in the national press" block on a country's Media tab; AI Analyst — the national-press line and evidence in country answers |
| `data/trends/ooni_censorship.json` | daily trend engine, via `scripts/fetch_ooni_censorship.py` (keyless; **CC BY-NC-SA licence — remove this layer if the Atlas is ever monetised**) | daily, after the 16:05 UTC run (rolling 28-day window) | Map — the "Confirmed news-site blocking" rows in Information freedom; AI Analyst — the measured-censorship line and evidence |
| `data/trends/attention_archive.json` | daily trend engine, via `scripts/update_attention_archive.py` (created once by its `--backfill` mode, 2026-08-18) | daily, after the 16:05 UTC run (no network — derived from the pageviews file) | Topic Explorer — custom trend windows whose start predates the live 120-day store: one GLOBAL daily total per topic back to 2024-01-01, fetched by the page only when such a window is actually picked. Per-language detail and quarantine screening exist only in the live window, and the page says so |
...
`data/trends/movers_feed.xml` is also not in the table because no page loads it: it is the Topic Explorer's daily movers list as an RSS feed, read by feed readers (Outlook, Teams, RSS apps) that visitors subscribe with. The Topic Explorer links to it ("Subscribe (RSS)" on the movers pane). It carries the published figures verbatim, so it can never disagree with the page.
...
**What to do.** Usually nothing: the fetchers work stalest-first, so consecutive runs claw back coverage on their own. Check `data/trends/topic_intelligence.json` → `coverage` after a few days; **`series_stale_scoreable` is the number that matters** and it should be falling (`series_stale_excluded` also counts a large dormant tail of low-traffic series that are refreshed only on Sundays by design and can never affect scores — that number being big is normal, not a fault; this distinction was added 2026-08-11 after the old alarm cried wolf on every run). If the scoreable count is still climbing after a week, the pageview fetch needs more parallel budget — raise the shard count (add `4` to the `shard:` list in `trend-engine.yml` AND change the two `3`s in the shard step's command to `4`, plus a fourth filename under the merge step), or raise `DORMANT_MEAN` in `scripts/fetch_trends_wikipedia.py` (fewer low-traffic series fetched on weekdays).
