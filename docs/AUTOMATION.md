...
| `data/trends/ooni_censorship.json` | daily trend engine, via `scripts/fetch_ooni_censorship.py` (keyless; **CC BY-NC-SA licence — remove this layer if the Atlas is ever monetised**) | every morning (rolling 28-day window) | Map — the "Confirmed news-site blocking" rows in Information freedom; AI Analyst — the measured-censorship line and evidence |
