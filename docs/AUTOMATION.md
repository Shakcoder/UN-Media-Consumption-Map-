...
| [`attention-archive-backfill.yml`](../.github/workflows/attention-archive-backfill.yml) | **Manual only** (Run workflow button) — ran once at launch (2026-08-18); run again only after adding topics to the registry, or if the archive file is ever lost | The Topic Explorer's long-term attention archive: global daily totals per topic back to 2024-01-01, fetched from the Wikimedia per-article API in 8 parallel shards (same own-IP-per-runner trick as the daily engine — one IP gets ~500 requests/hour before throttling, the backfill needs ~3,400). The assemble job refuses to write anything incomplete, golden-checks the result against `wiki_pageviews.json`, validates, and commits. After that the daily engine keeps the file current | `data/trends/attention_archive.json` |
