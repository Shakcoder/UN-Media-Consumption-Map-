...
|---|---|---|---|
| [`trend-engine.yml`](../.github/workflows/trend-engine.yml) | Daily 05:30 UTC | Each country's trending Google searches (Trending Now RSS — runs first: the feed keeps no history, so a missed day is gone); Wikipedia pageviews (demand) + GDELT coverage (supply) → per-topic and per-country trend intelligence; plus each country's most-read Wikipedia pages (directly measured, Wikimedia top-per-country) | `data/trends/country_searches.json`, `data/trends/wiki_pageviews.json`, `data/trends/gdelt_coverage.json`, `data/trends/topic_intelligence.json`, `data/trends/country_reading.json` |
