...
| Google Trends "Trending Now" RSS | each country's ~10 trending search queries with Google's traffic buckets, plus a 7-day rolling archive kept in-repo (`data/trends/country_searches.json`, `scripts/fetch_trends_google.py`) | **121/195 on the first full run (2026-08-10)** — Google publishes no feed for the other ~74 (HTTP 400); they carry an explicit **unsupported** flag and are never estimated | **Automated daily** (trend engine; deliberately the first step — the feed is a snapshot with no upstream history, so a missed day is unrecoverable) | Google Trends data, reused with attribution ("Data source: Google Trends") per Google's "Export, embed and cite" guidance; **only derived facts committed** (query, bucket, date) — the feed's third-party headlines and images are deliberately dropped | ✅ added 2026-08-10 — **search interest of any kind, not news interest**: sports and TV shows sit next to elections, and that is the honest picture. Traffic buckets ("500+", "20K+") are floors, never counts — never summed, never compared across countries as volumes |
