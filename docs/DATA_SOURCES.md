...
| UN Google Analytics (GA360) — UN News properties | measured site readership (`data/ga_summary.json`): English-edition totals + trend, per-country active users/engagement (top 30), devices, arrival channels, top sources, top pages (14-day windows), browser locales, per-window aggregates (28d / prev-28d / 90d), monthly history to 2020-09, hour/day-of-week timing, landing pages, partial demographics (classifiable subset only) | GA reports 238 countries/territories; 30 published per-country | Manual aggregate pull per reporting cycle (API-assisted equivalents of the six approved Explore reports — see `docs/GA_SUMMARY_EXPORTS.md`) | **Internal UN operational data — not an open dataset.** Published as aggregated summaries only, per DGC approval 2026-07-30; scope broadened per Fang Chen's verbal go-ahead relayed 2026-08-04 (written confirmation pending); raw/event/user-level exports never enter this public repo (`.gitignore` + `validate_atlas.py` raw-field guard) | ✅ added 2026-08-04 — **arrivals construct**: measures visits to news.un.org, never a country's reachable population; carries its own `[un-traffic]` tag in the analyst and a construct caveat on every surface. China row bot-flagged; 54% of sessions unattributed (channel shares quoted on attributed traffic) |
