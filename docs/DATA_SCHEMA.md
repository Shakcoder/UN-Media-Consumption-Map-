> **⚠ Historical document (April 2026).** The shipped schema evolved past this draft — `data/countries.json` now uses schema v2 with nested `demographics` / `connectivity` / `information_freedom` / `news_consumption` / `languages_detail` groups and per-field `sources`. For the current authoritative source registry see [DATA_SOURCES.md](DATA_SOURCES.md); for the live structure, open `data/countries.json` and inspect any country record.
