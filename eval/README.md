...
## `node scripts/run_eval.mjs selfknowledge` — conversational coverage

Added 2026-07-28. The golden-100 asks well-formed questions; this suite asks
the way people actually type, including questions about the tool itself
("what can you do", "how does this work", "how confident are you"). It
enforces two things:

- **No dead ends.** A generic "I couldn't match that" fails the suite. An
  honest "the Atlas holds no media prices, here is the nearest real evidence"
  passes — declining is a feature, being unable to route is not.
- **Every answer can explain itself.** Each response must carry a reasoning
  trace (what was recognised, which route was chosen, what it drew on).

Currently 28/28 routed with 0 dead ends. It runs in CI on every push.
